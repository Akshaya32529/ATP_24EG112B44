...
function ListOfEmp() {
  ...
  async function getEmps() {
    try {
      setLoading(true)
      setError("")
      let res = await fetch("https://atp-24eg112b44-2.onrender.com/employee-api/employees");
      if (res.status === 200) {
  ...
  const deleteEmp = async (id) => {
    try {
      setLoading(true)
      setError("")
      let res = await axios.delete(`https://atp-24eg112b44-2.onrender.com/employee-api/employees/${id}`)
      if (res.status === 200) {
        getEmps()
      }
    } catch (err) {
